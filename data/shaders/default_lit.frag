...
vec3 calc_direct_light(const vec3 albedo, const vec3 normal) {
    ...
    PointLight point_lights[1];
    point_lights[0].pos = vec3(0.0, 0.45, 0.0);
    point_lights[0].col = vec3(1.0);
    ...
    for(uint i=0; i<num_point_lights; ++i) {
        ...
        // diffuse += normal * att * pl.col * mat.diffuse_color.a;
...
vec4 indirectDiffuse(vec3 albedo, vec3 normal) {
    const float voxel_size = 2.0 / 256.0;
...
void main() {
    ...
    vec4 specular = specular_cone(frag_pos, normalize(reflect(normalize(frag_pos - view_pos), normal)));
    vec4 indirect = indirectDiffuse(albedo, normal);
    // indirect.rgb = pow(indirect.rgb, vec3(1.0 / 2.2));
    // indirect.rgb *= 0.0;
    vec3 direct = calc_direct_light(albedo, normal);
    // direct *= 0.0;
