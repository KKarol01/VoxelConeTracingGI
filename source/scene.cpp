...
static Texture2D get_asset_texture(const fastgltf::Asset* asset, u64 texture_index, std::unordered_map<std::filesystem::path, Texture2D>* cache) {
    ...
    if(const auto* array = std::get_if<fastgltf::sources::Array>(&image.data)) {
        ...
        auto& new_tex = cache->insert({
            image.name,
            Texture2D{std::format("scene_texture_2d_{}", image.name), (u32)x, (u32)y, vk::Format::eR8G8B8A8Unorm, (u32)std::log2f(std::min(x, y)) + 1, vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eTransferSrc, std::span{raw_image, x*y*4u}}
